...
public class ExcelHelpers {
    ...
    //Get last row number (get the row number of the last row)
    public int getLastRowNum() {
        return sheet.getLastRowNum();
    }

    //Get physical number of rows (get the number of rows used)
    ...
    public void setCellData(String text, int rowNumber, int colNumber) {
        try {
            ...
            } else if (text.equals("fail") || text.equals("failed")) {
    ...
    //There is only 1 Field in the Excel File.
    public Object[][] getExcelData(String fileName, String sheetName) {
        ...
        try {
            ...
            //FOR loop runs from 1 to remove the title line (title line is 0)
            for (int i = 1; i < noOfRows; i++) {
                for (int j = 0; j < noOfCols; j++) {
                    row = sheet.getRow(i);
                    cell = row.getCell(j);

                    //This function is used to determine the data type from a cell in Excel and then convert it to a String for easy reading.
                    switch (cell.getCellType()) {
                        case STRING:
    ...
    //For cases with multiple fields in an Excel File.
